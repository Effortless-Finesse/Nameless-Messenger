...
gboolean is_chat_maximized = TRUE; //chat header maximization
const gchar *finalname;
static gchar *last_sender = NULL;
...
void add_chat_message(GtkWidget *text_view, const char *name, const char *msg) {
    ...
    if (!last_sender || g_strcmp0(finalname, last_sender) != 0) {
        ...
        g_free(last_sender);
        last_sender = g_strdup(finalname);
    } else { 
        gtk_text_buffer_insert_with_tags_by_name(buffer, &end, "        ", -1, "message", NULL);
    }
...
void on_send_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (g_strcmp0(msg, "") != 0) {
        add_chat_message(widgets->chat_display, "Alice", msg);
        gtk_entry_set_text(GTK_ENTRY(widgets->entry), "");  // clear entry
    }
}

void on_connect_clicked(GtkWidget *widget, gpointer entry) {
    const gchar *name = gtk_entry_get_text(GTK_ENTRY(entry));

    if (g_strcmp0(name, "") == 0) {
        gtk_entry_set_text(GTK_ENTRY(entry), "ObnoxiousFASTstudentDickheadx");
        g_print("No Name entered - naming you ObnoxiousFASTstudentDickheadx.\n");
    }

    finalname = gtk_entry_get_text(GTK_ENTRY(entry));

    rust_bridge_start( rust_bridge_send, NULL, finalname);
...
int main(int argc, char *argv[]) {
    ...
    g_signal_connect(btn, "clicked", G_CALLBACK(on_connect_clicked), entry);
    g_signal_connect(entry, "activate", G_CALLBACK(on_connect_clicked), entry);
